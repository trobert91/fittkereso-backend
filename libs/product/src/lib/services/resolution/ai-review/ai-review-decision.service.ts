...
function formatCandidate(candidate: AiReviewCandidate): string {
  ...
  if (live.specs && !isEmpty(live.specs)) {
    lines.push(
      `- Live specs: ${Object.entries(live.specs)
  ...
  if (candidate.specMatchDetails) {
    const specs = candidate.specMatchDetails;
    lines.push(
      `- Matcher spec comparison: ${specs.matchingCount}/${specs.comparableCount} matched, ${specs.primaryMismatches} primary mismatch(es)`,
    );
  }
