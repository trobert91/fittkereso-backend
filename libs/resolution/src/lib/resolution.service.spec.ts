...
describe('ResolutionService.search()', () => {
  ...
  describe('recording the recall pool', () => {
    ...
    function makeFilteredRunService() {
      const recallService = {
        recall: jest.fn().mockImplementation(async (ctx: ResolutionContext) => {
          if (ctx.strategiesRun.length === 0) {
            ctx.strategiesRun.push('fuzzy');
    ...
    it('keeps the filter rejection with its mismatch detail on the context', async () => {
      const result = await makeFilteredRunService().search(INPUT, OPTIONS);

      expect(result.context.filter?.filteredCandidates).toHaveLength(1);
      expect(result.context.filter?.filteredCandidates?.[0]).toMatchObject({
        candidateId: 'cand-1',
        candidateName: 'KTM MACINA KAPOHO ELITE',
        reason: 'match_specs',
        detail: MISMATCH_DETAIL,
    ...
    it('keeps survivors\u2019 matcher scores on the recorded pool', async () => {
      ...
      const result = await service.search(INPUT, OPTIONS);

      const pool = result.context.recallCandidates ?? [];
      expect(pool).toHaveLength(2);

      const byId = Object.fromEntries(
        pool.map((candidate) => [candidate.productId, candidate]),
      );
      expect(byId['cand-keep'].matchScore).toBe(72);
      expect(byId['cand-1'].matchScore).toBeUndefined();
      expect(result.context.filter?.filteredCandidates?.[0]?.detail).toBe(
        MISMATCH_DETAIL,
      );
    });
  });
